...
% ------- test data -----
testInput = allData(sampSize:end,:)';
testGround = TargetLabel(sampSize:end,:)';



% --------- break dataSet, continue train on previous weight --------
eerMdegs = [];
tS       = [];

nSL = 10;
splitSize = floor(sampSize/nSL);
for nS = 0:nSL-1
...
%     Targets = TargetLabel(1:sampSize,:)';
    ...
    Inputs = allData(nS*splitSize+1:(nS+1)*splitSize,:)';
    Targets = TargetLabel(nS*splitSize+1:(nS+1)*splitSize,:)';
